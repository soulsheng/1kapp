...
function wp_install( $blog_title, $user_name, $user_email, $public, $deprecated = '', $user_password = '' ) {
	...
	$guessurl = isset($_SERVER['HTTP_APPNAME'])?'http://'.substr($_SERVER['HTTP_APPNAME'],5).'.1kapp.com':wp_guess_url();//[ysd] 固定了guessurl
	...
	if(!file_exists(ABSPATH.'wp-admin/without_default')) wp_install_defaults($user_id); //[ysd]，如果打包时设置了默认数据，才会设置默认数据
...
function wp_install_defaults($user_id) {
	...
	if ( global_terms_enabled() ) {
		...
		$cat_id = 1001;//[ysd] 增大主键，防止于导出数据的主键重复
	}
	...
	if ( global_terms_enabled() ) {
		...
		$blogroll_id = 1002;//[ysd] 增大主键，防止于导出数据的主键重复
	}
